...
int laser_array_init(laser_array_t *la, const laser_array_config_t *config) {
    ...
    RETURN_ON_ERROR(HAL_DMA_Start_IT(la->config.htim_transfer->hdma[TIM_DMA_ID_UPDATE], (uint32_t) la->tx_data,
    ...
    RETURN_ON_ERROR(HAL_TIM_Base_Start_IT(la->config.htim_fade), "Failed to start fade update timer");
...
int laser_array_set_brightness(laser_array_t *la, uint8_t diode_index, uint8_t brightness) {
    ...
    RETURN_ON_FALSE(diode_index < LA_NUM_DIODES, RET_INVALID_PARAM, "Invalid diode index: %u", diode_index);
